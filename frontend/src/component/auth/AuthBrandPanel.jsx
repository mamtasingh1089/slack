...
const AuthBrandPanel = () => {
  ...
  // Smooth springs for tracking the mouse position (increased damping for slower, calmer tracking response)
  const springX = useSpring(mouseX, { stiffness: 35, damping: 18 });
  const springY = useSpring(mouseY, { stiffness: 35, damping: 18 });

  // Map Springs to translation distances for 3D Parallax Depth Layers (reduced ranges for slower/subtler horizontal motion)
  ...
  const handleMouseLeave = () => {
    // Reset to center smoothly when mouse leaves the panel
    mouseX.set(0);
    mouseY.set(0);
  };
  ...
  return (
    <div
      ...
      className="hidden lg:flex lg:w-1/2 relative flex-col justify-between overflow-hidden border-r border-[#DFE7E3] h-screen bg-[#F8FAF9] pt-12 px-12 xl:pt-16 xl:px-16 pb-0 select-none cursor-default"
    >
      
      {/* Morning Dawn-to-Daylight Sky Transition Layer */}
      {!shouldReduceMotion && (
        <motion.div
      ...
      <motion.img
        ...
        animate={shouldReduceMotion ? { opacity: 0.9 } : { opacity: 0.9, x: 0 }}
        transition={{ duration: 1.5, ease: "easeOut" }}
      />

      {/* Grid Dots decorations in the background */}
      <motion.div
        className="absolute top-8 left-8 flex flex-col gap-1.5 z-0 pointer-events-none"
        initial={{ opacity: 0 }}
      ...
      <motion.div
        className="absolute top-[40%] left-6 flex flex-col gap-1.5 z-0 pointer-events-none"
        initial={{ opacity: 0 }}
        animate={shouldReduceMotion ? { opacity: 0.25 } : { opacity: [0.35, 0.15, 0.35] }}
      ...
      {/* Cloud Cluster on the Right Side (Tracks mouse for subtle depth) */}
      <motion.img
        src={Cloud1}
        alt=""
        className="absolute right-[-70px] bottom-[14%] w-[260px] xl:w-[300px] opacity-95 pointer-events-none z-0"
        style={shouldReduceMotion ? {} : { x: parallaxBgCloudX, y: parallaxBgCloudY }}
        initial={shouldReduceMotion ? {} : { scale: 0.8, opacity: 0 }}
        animate={shouldReduceMotion ? { scale: 1, opacity: 0.95 } : { scale: 1, opacity: 0.95 }}
        transition={{ duration: 2, ease: "easeOut" }}
      />
      <motion.img
        src={Cloud2}
        alt=""
        className="absolute right-[70px] bottom-[6%] w-[180px] xl:w-[210px] opacity-95 pointer-events-none z-0"
        style={shouldReduceMotion ? {} : { x: parallaxBgCloudX, y: parallaxBgCloudY }}
        initial={shouldReduceMotion ? {} : { scale: 0.8, opacity: 0 }}
        animate={shouldReduceMotion ? { scale: 1, opacity: 0.95 } : { scale: 1, opacity: 0.95 }}
      ...
      {/* UPPER SECTION: Big 9 (BoyGirl) + AM Logo & Brand Details (Reduced gap between illustration and text columns) */}
      <motion.div
      ...
      <div className="relative flex flex-col justify-center flex-grow mt-8 pl-6 pr-6 pb-[240px] xl:pb-[280px] z-10">
        ...
        <motion.div
          className="flex flex-col gap-3 max-w-[340px] xl:max-w-[420px]"
      ...
      {/* BOTTOM SECTION: Landscape Layered SVGs */}
      <div className="absolute bottom-0 left-0 right-0 w-full h-[220px] xl:h-[260px] pointer-events-none select-none z-10 overflow-hidden">
        ...
        {/* Layer 1: Fourth Hill (backmost, light mint/teal) - ONLY horizontal slide */}
        <motion.img
          src={FourthHill}
          alt=""
          className="absolute bottom-0 left-0 w-full h-auto object-cover object-bottom z-10 scale-x-105"
        ...
        {/* Layer 2: Third Hill (medium teal) - ONLY horizontal slide */}
        <motion.img
          src={ThirdHill}
          alt=""
          className="absolute bottom-0 left-0 w-full h-auto object-cover object-bottom z-20 scale-x-105"
        ...
        {/* Layer 3: First Hill (darker teal/blue) - ONLY horizontal slide */}
        <motion.img
          src={FirstHill}
          alt=""
          className="absolute bottom-0 left-0 w-full h-auto object-cover object-bottom z-30 scale-x-105"
        ...
        {/* Layer 4: Second Hill (foreground, darkest blue-green) - ONLY horizontal slide */}
        <motion.img
          src={SecondHill}
          alt=""
          className="absolute bottom-0 left-0 w-full h-auto object-cover object-bottom z-40 scale-x-105"
        ...
        {/* Layer 5: Left & Right framing leaves (Sway + Parallax) */}
        <motion.img
          src={LeftLeaves}
          alt=""
